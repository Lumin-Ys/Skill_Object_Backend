...
const ROOT_DOMAIN = process.env.ROOT_DOMAIN || 'aeladder.store';
const PUBLIC_ORIGIN = (process.env.PUBLIC_ORIGIN || `http://${ROOT_DOMAIN}`).replace(/\/+$/, '');
  ...
  return storage.update((db) => {
    ...
    for (const task of Object.values(db.tasks || {})) {
      ...
      if (task.status === 'success') {
        task.url = `${PUBLIC_ORIGIN}/sites/${task.id}/`;
      }
    }
    return publicUser(user);
  });
