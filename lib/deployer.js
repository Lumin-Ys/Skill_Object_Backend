 ...
 *   5. 返回完整 URL：http://aeladder.store/sites/<taskId>/
 */
...
const { ROOT_DOMAIN, PUBLIC_ORIGIN } = require('./auth');
...
function getTask(taskId) {
  ...
  return presentTask(db.tasks[taskId] || null);
}
...
function hostForTask(task) {
  if (task.subdomain) return `${task.subdomain}.${ROOT_DOMAIN}`;
  const slug = String(task.domain || '').split('.')[0];
  return slug ? `${slug}.${ROOT_DOMAIN}` : ROOT_DOMAIN;
}

function taskPublicUrl(task) {
  return `${PUBLIC_ORIGIN}/sites/${task.id}/`;
}
...
function presentTask(task) {
  ...
  return {
    ...task,
    domain: hostForTask(task),
    url: task.status === 'success' ? taskPublicUrl(task) : task.url,
  };
}
...
function createTask(user, zipPath) {
  ...
  const task = {
    ...
    userId: user.id,
    userName: user.name,
    domain: `${user.slug || 'app'}.${ROOT_DOMAIN}`,
    status: 'pending',
    message: '任务已创建，等待调度',
    progress: 0,
    zipPath,
